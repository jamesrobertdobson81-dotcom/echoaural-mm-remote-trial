...
test('hasClassroomAdapter matches the adapters classroom-server.js actually registers today', () => {
  // The adapter registration list lives in classroom/adapters.js (shared
  // with accounts/account-server.js's homework routes), not inlined in
  // classroom-server.js itself — see that file's own createAdapters().
  const serverSource = fs.readFileSync(path.join(root, 'classroom/adapters.js'), 'utf8');
  // Sanity-checked against the real registration list in
  // createAdapters() — if this regexp stops matching, that function's
  // shape has changed and this test (not just the registry) needs a look,
  // not a silent skip.
  ...
  assert.equal(withAdapter.length, 17);
});
