...
test('extractConceptValues: melody-master buckets correctAnswer into a specific device name alongside category', () => {
  const values = CE.extractConceptValues('melody-master', { category: 'Melodic device', correctAnswer: 'Descending sequence' });
  assert.deepEqual(values, ['Melodic device', 'Sequence']);
});

test('extractConceptValues: melody-master device-name bucket matches every real device the live bank uses', () => {
  assert.equal(CE.extractConceptValues('melody-master', { correctAnswer: 'Ascending sequence' })[0], 'Sequence');
  assert.equal(CE.extractConceptValues('melody-master', { correctAnswer: 'An ostinato' })[0], 'Ostinato');
  assert.equal(CE.extractConceptValues('melody-master', { correctAnswer: 'A repeated short riff/ostinato' })[0], 'Ostinato');
  assert.equal(CE.extractConceptValues('melody-master', { correctAnswer: 'Repeated notes' })[0], 'Repeated notes');
  assert.equal(CE.extractConceptValues('melody-master', { correctAnswer: 'Scalic' })[0], 'Scalic/stepwise movement');
  assert.equal(CE.extractConceptValues('melody-master', { correctAnswer: 'Mainly by leap, with some stepwise movement' })[0], 'Scalic/stepwise movement');
  assert.equal(CE.extractConceptValues('melody-master', { correctAnswer: 'Maqām' })[0], 'Maqam');
});

test('extractConceptValues: melody-master device-name bucket never fires "Ornament" (already covered by category, avoids double-counting one answer)', () => {
  // Trill/turn/mordent-type correctAnswers must NOT also produce "Ornament"
  // via the device bucket, since the category dimension already can.
  assert.deepEqual(CE.extractConceptValues('melody-master', { category: 'Ornament', correctAnswer: 'Trill' }), ['Ornament']);
  assert.deepEqual(CE.extractConceptValues('melody-master', { correctAnswer: 'Turn' }), []);
});

test('extractConceptValues: melody-master device-name bucket ignores idiosyncratic free-text answers it can\'t classify', () => {
  assert.deepEqual(CE.extractConceptValues('melody-master', { correctAnswer: 'A raga' }), []);
  assert.deepEqual(CE.extractConceptValues('melody-master', { correctAnswer: 'It begins with broken-chord or arpeggio movement, then moves by step in a scale.' }), ['Scalic/stepwise movement']);
});

test('extractConceptValues: melody-master aosCode buckets into 3 tradition groups', () => {
  assert.deepEqual(CE.extractConceptValues('melody-master', { aosCode: 'AoS1' }), ['Western classical tradition']);
  assert.deepEqual(CE.extractConceptValues('melody-master', { aosCode: 'AoS2' }), ['Western classical tradition']);
  assert.deepEqual(CE.extractConceptValues('melody-master', { aosCode: 'AoS3' }), ['Western classical tradition']);
  assert.deepEqual(CE.extractConceptValues('melody-master', { aosCode: 'AoS6' }), ['World and non-Western traditions']);
  assert.deepEqual(CE.extractConceptValues('melody-master', { aosCode: 'AoS4' }), ['Popular, dance and screen music']);
  assert.deepEqual(CE.extractConceptValues('melody-master', { aosCode: 'AoS5' }), ['Popular, dance and screen music']);
  assert.deepEqual(CE.extractConceptValues('melody-master', { aosCode: 'AoS7' }), ['Popular, dance and screen music']);
});

test('extractConceptValues: melody-master aosCode is silently absent (not an error) when the client-side AoS map fetch never resolved', () => {
  assert.deepEqual(CE.extractConceptValues('melody-master', { category: 'Mode/Scale' }), ['Mode/Scale']);
});

test('extractConceptValues: melody-master pools category, difficulty, device name and AoS tradition together from one full devices answer', () => {
  const values = CE.extractConceptValues('melody-master', {
    category: 'Melodic device',
    correctAnswer: 'Descending sequence',
    aosCode: 'AoS2'
  });
  assert.deepEqual(values, ['Melodic device', 'Sequence', 'Western classical tradition']);
});
