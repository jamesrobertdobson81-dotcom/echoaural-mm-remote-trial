...
(function (root, factory) {
  ...
  // melody-master-devices' `correctAnswer` is free display text, not a clean
  // category field (unlike meter-master's metreFamily or texture-trainer's
  // textureFocus) — keyword-matched into buckets rather than whitelisted
  // directly, same reason ornamentTypeTier above buckets rather than
  // whitelists individual ScoreDecoder terms. Verified against the real
  // 92-question bank: these 5 buckets each have 6+ underlying questions
  // (Sequence 15, Scalic/stepwise 14, Ostinato 7, Repeated notes 7, Maqam
  // 6); the remaining ~19 answers are either one-off descriptive sentences
  // from the pop-culture batch (e.g. "It begins with broken-chord or
  // arpeggio movement...") that don't reduce to a reusable device name, or
  // (trill/turn/mordent-type answers) already covered at the right
  // granularity by the existing `category` dimension's own "Ornament"
  // value above — deliberately NOT also bucketed here under the same
  // string, which would silently double-count that one answer under one
  // concept key (the exact collision chordFunctionTier's own comment
  // documents avoiding). Both exclusions follow the same "exclude the
  // sparse/already-covered long tail" discipline as METER_MODE_WHITELIST.
  // Order matters: more specific patterns are checked before broader ones
  // (e.g. "repeated short riff/ostinato" must hit Ostinato, not a generic
  // repetition bucket) since a bucket function returns on first match.
  function melodicDeviceTier(correctAnswer) {
    var value = String(correctAnswer || "").toLowerCase();
    if (/sequence/.test(value)) return "Sequence";
    if (/ostinato|repeated short riff|looping/.test(value)) return "Ostinato";
    if (/repeated note|repeated pitch/.test(value)) return "Repeated notes";
    if (/scalic|stepwise|by step|scale/.test(value)) return "Scalic/stepwise movement";
    if (/maqam|maq[āa]m/.test(value)) return "Maqam";
    return null;
  }

  // Groups a melody-master-devices question's CIE Area of Study code (see
  // shared/data/question-aos-map.json, joined in client-side by
  // modules/melody-master/script.js before this extractor ever runs — see
  // that file's own comment on why the join happens there rather than here)
  // into 3 broad traditions, so feedback can independently name "you're
  // weaker on world-tradition devices" separately from which specific
  // device, when that's the clearer signal. Verified real coverage against
  // the 92-question bank: Western classical (AoS1 Baroque + AoS2 Classical +
  // AoS3 Romantic) 47, World and non-Western (AoS6) 19, Popular/dance/screen
  // (AoS4 words + AoS5 dance + AoS7 stage-and-screen) 26 — all comfortably
  // clear the sample-size bar. AoS4/5/7 pooled together rather than kept
  // separate: none of the three alone would be a large or especially
  // meaningful bucket for THIS specific bank (melodic-device recognition),
  // unlike AoS6's genuinely distinct world-tradition ornamentation
  // vocabulary (meend/maqam/raga) that's worth calling out on its own.
  function aosTraditionTier(aosCode) {
    var value = String(aosCode || "");
    if (value === "AoS1" || value === "AoS2" || value === "AoS3") return "Western classical tradition";
    if (value === "AoS6") return "World and non-Western traditions";
    if (value === "AoS4" || value === "AoS5" || value === "AoS7") return "Popular, dance and screen music";
    return null;
  }
  ...
  var CONCEPT_EXTRACTORS_BY_MODULE = {
    ...
    "melody-master": [
      ...
      // Both devices-only: dictation answers carry neither field, so these
      // simply extract nothing (null) for dictation questions — safe, no
      // special-casing needed since they pool in the same moduleId.
      { field: "correctAnswer", bucket: melodicDeviceTier },
      { field: "aosCode", bucket: aosTraditionTier }
    ],
    "cadence-coach": [
