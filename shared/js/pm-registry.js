...
(function (root, factory) {
  ...
  function entry(sourceKey, moduleId, area, subAppLabel, appUrl, levelValues, hasClassroomAdapter, rendererBridgeCapability, questionSelectionMode, audioContract) {
    ...
    return {
      ...
      // Every source here is a listening exercise built around audio
      // clips — true for all of them as of this writing, but kept as an
      // explicit field (not an assumption baked into behaviour) so a
      // future fully text-only source doesn't silently inherit it. Defaults
      // to true; pass false explicitly for a source with no audio at all.
      audioContract: audioContract !== false,
