...
class RoomManager {
  ...
  createRoom({ moduleId, baseUrl, apiBase = '' }) {
    this.cleanupExpiredRooms();
    const adapter = this.getAdapter(moduleId);
    const code = this.makeRoomCode();
    const sharedUrlParams = apiBase ? { classroomApi: apiBase } : {};
    const shortJoinUrl = buildClassroomUrl(baseUrl, '/join', sharedUrlParams);
  ...
  joinRoom(room, studentId, name) {
    const validation = this.validateStudentName(room, studentId, name);
    if (!validation.ok) return validation;
    room.students.set(studentId, { id: studentId, name: validation.name, lastSeen: Date.now() });
