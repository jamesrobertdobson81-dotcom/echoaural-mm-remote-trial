...
function createClassroomServer(options = {}) {
  ...
  async function handleApi(req, res, parsedUrl) {
    try {
      ...
      if (req.method === 'GET' && pathname === '/api/classroom/modules') {
        return sendJson(res, 200, { ok: true, modules: roomManager.getModules() });
      }
      ...
      if (req.method === 'POST' && pathname === '/api/classroom/create') {
        ...
        const frontendBase = normalizePublicBaseUrl(process.env.PUBLIC_SITE_URL)
          || normalizePublicBaseUrl(body.frontendBase)
          || normalizePublicBaseUrl(body.publicSiteUrl)
          || getBaseUrl(req);
        const apiBase = normalizePublicBaseUrl(process.env.PUBLIC_API_URL)
          || normalizePublicBaseUrl(body.apiBase);
      ...
      if (req.method === 'POST' && pathname === '/api/classroom/module') {
        ...
        return sendJson(res, 200, { ok: true, state: roomManager.createRoomState(room) });
      }
      ...
      if (req.method === 'POST' && pathname === '/api/classroom/start') {
        ...
        return sendJson(res, 200, { ok: true, question, state: roomManager.createRoomState(room) });
      }
      ...
      if (req.method === 'POST' && pathname === '/api/classroom/next') {
        ...
        if (room.quizStarted && Number(room.quizQuestionNumber || 0) >= Number(room.quizTotal || 1)) {
          ...
          return sendJson(res, 400, { ok: false, error: 'Quiz complete. End the quiz or adjust settings for a new one.', state: roomManager.createRoomState(room) });
        }
      ...
      if (req.method === 'POST' && pathname === '/api/classroom/join') {
        ...
        const joined = roomManager.joinRoom(room, studentId, body.name);
      ...
      if (req.method === 'POST' && pathname === '/api/classroom/submit') {
        ...
        try {
          ...
          return sendJson(res, 200, { ok: true, submission, state: roomManager.createRoomState(room, studentId) });
        } catch (error) {
          const status = error.code === 'ALREADY_SUBMITTED' || error.code === 'SUBMISSIONS_CLOSED' ? 400 : 404;
          return sendJson(res, status, { ok: false, error: error.message, state: roomManager.createRoomState(room, studentId) });
        }
      }
  ...
  function logStartup() {
    ...
    console.log(`Teacher on this laptop: http://localhost:${port}/teacher/`);
