...
function saveTextureProgress(percentage) {
  ...
  window.EchoAuralTracking.saveRound({
    ...
    roundFeedback: percentage >= 85
      ? "Secure texture vocabulary. Keep supporting each term with precise audible evidence."
      : percentage >= 60
        ? "Your texture vocabulary is developing. Name the texture first, then explain the relationship between the musical lines."
        : "Keep practising concise GCSE answers: identify the texture, then give one clear piece of audible evidence.",
    metadata: { medal: medal.title },
