(() => {
  ...
  const SCORE_ASSET = 'assets/blank-treble-bar.png';
  ...
  function buildRound() {
    ...
    return questions.map((question) => ({ ...question, mode }));
  }
  ...
  function stopIntervalAudio() {
    ...
    try {
      ...
      // Some browsers can object if the source has not fully loaded yet. Safe to ignore.
    }
    audioPlaying = false;
  }
  ...
  async function playInterval() {
    ...
    els.feedback.textContent = 'Listen for the size of the melodic leap.';
  ...
  function viewToPixels(x, y) {
    const rect = els.staveStage.getBoundingClientRect();
    return {
      left: (x / STAVE_WIDTH) * rect.width,
      top: (y / STAVE_HEIGHT) * rect.height
    };
  }
  ...
  function attachDragHandlers() {
    ...
    els.noteHint.textContent = 'Drag the crotchet to the second pitch you heard, then name the interval.';
    ...
    dragNote.addEventListener('pointermove', (event) => {
      ...
      positionDragNote(draggedNoteId);
      els.noteHint.textContent = `Second note placed on ${nearest.label}. Now choose the interval name.`;
      updateCheckButton();
    });
  ...
  function renderStave(question) {
    ...
    if (hideTarget) {
      ...
      els.noteHint.textContent = `${question.startNoteLabel} to ${question.targetNoteLabel}. Listen, then name the interval.`;
    }
  }
  ...
  function setQuestion(question) {
    ...
    els.xpText.textContent = question.mode === 'construction' ? 'Pitch + interval' : 'Interval name';
    ...
    els.answerCard.innerHTML = `
      <div class="answer-reveal mm-source-panel ${isFullyCorrect ? 'is-correct' : 'is-wrong'} mm-diagnostic-panel mm-gcse-feedback-panel mm-main-quiz-feedback-panel mi-console-feedback-panel">
        ...
        <div class="diagnostic-card mm-round-score-tile ${questionIndex >= roundQuestions.length - 1 ? 'is-final' : ''}">
          ...
          <p>${escapeHTML(`${answerDetail} · Correct: ${question.intervalLabel} · ${question.startNoteLabel} → ${question.targetNoteLabel}`)}</p>
        </div>
      </div>
    `;
  }
  ...
  function renderRoundQuestionRows() {
    ...
    return roundResults.map((result, index) => {
      ...
      const detail = result.mode === 'construction'
        ? `${result.startNoteLabel} → ${result.targetNoteLabel} · ${result.intervalLabel} · ${result.pitchCorrect ? 'pitch secure' : `placed ${result.draggedNoteLabel || '—'}`}`
        : `${result.startNoteLabel} → ${result.targetNoteLabel} · ${result.intervalLabel}`;
      return `
  ...
  function checkAnswer() {
    ...
    const intervalCorrect = data.sameInterval(selectedInterval, currentQuestion.intervalLabel);
    ...
    els.answers.querySelectorAll('.mi-answer-button').forEach((button) => {
      ...
      if (data.sameInterval(value, currentQuestion.intervalLabel)) button.classList.add('correct');
      if (data.sameInterval(value, selectedInterval) && !data.sameInterval(value, currentQuestion.intervalLabel)) button.classList.add('wrong');
    });

    const message = awarded === possible
      ? `Correct — ${currentQuestion.startNoteLabel} to ${currentQuestion.targetNoteLabel} is a ${currentQuestion.intervalLabel}.`
      : currentQuestion.mode === 'construction'
        ? `Not quite. The target was ${currentQuestion.targetNoteLabel}, creating a ${currentQuestion.intervalLabel}.`
        : `Not quite. The interval was a ${currentQuestion.intervalLabel}.`;
