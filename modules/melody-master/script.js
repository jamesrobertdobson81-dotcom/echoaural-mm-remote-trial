...
// Every entry here is a genuine per-level variant (sourceQuestionId + level),
// whether machine-generated by tools/generate-melody-master-clips.js's
// missing-note ladder or, for a couple of sources, hand-supplied to replace
// the generated one — the "manualLevelVariant" flag only distinguishes the
// latter kind, and only 4 of the 43 variants ever carried it. Filtering on
// that flag here meant 39 already-generated, level-appropriate variants
// (covering nearly every source melody) were silently never used — every
// level defaulted to the same one generic version of each melody. Use the
// whole set; a hand-supplied variant simply appears once, same as any
// generated one, since neither the generator nor the manual pass ever
// produces two variants for the same sourceQuestionId + level.
const ALL_MELODY_CLIPS = [...SOURCE_MELODY_CLIPS, ...LEVELLED_MELODY_CLIPS];
...
function getDictationQuestionIndexes() {
  ...
  const matchingLevelVariants = LEVELLED_MELODY_CLIPS.filter((clip) => clip.level === selectedLevel);
  const replacedSourceIds = new Set(matchingLevelVariants.map((clip) => clip.sourceQuestionId));

  return ALL_MELODY_CLIPS
    .map((clip, index) => {
      if (clip.sourceQuestionId) return clip.level === selectedLevel ? index : -1;
      return replacedSourceIds.has(clip.id) ? -1 : index;
    })
