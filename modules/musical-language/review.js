(function () {
  ...
  const STORAGE_KEY = "echoaural.musicalLanguage.review.v1";
  const DATA_URL = "data/EA_Musical_Language_v1.csv";
  const APP_URL = "index.html";
  const LEVELS = ["Foundation", "Developing", "Securing", "Mastering"];

  // Mirrors shared/js/pm-registry.js's per-source area assignment (tempo ->
  // rhythm/meter, dynamics -> texture, articulation & ornamentation ->
  // melody). Shown as a hint only — reviewing per-question is the point of
  // this tool, so the checkboxes themselves always start unchecked rather
  // than silently pre-answering the review.
  const ELEMENT_SUGGESTED_AREA = {
    Tempo: "Meter",
    Dynamics: "Texture",
    Articulation: "Melody",
  ...
  const ELEMENT_SOURCE_KEY = {
    Tempo: "musical-language-tempo",
    Dynamics: "musical-language-dynamics",
    Articulation: "musical-language-articulation",
    Ornamentation: "musical-language-ornamentation"
  };

  // Same hand-rolled quoted-CSV parser as modules/musical-language/script.js
  // (kept in sync deliberately rather than shared, so this review tool has
  // no runtime dependency on the app's own script).
  ...
  const elements = {
    ...
    areaSuggestion: document.getElementById("areaSuggestion"),
  ...
  function reviewIsComplete(review = {}) {
    return Boolean(review.decision || review.difficulty || review.notes || (review.areas || []).length || (review.issues || []).length);
  }
      ...
      ? rows.map(({ question, index }) => {
        ...
        return `
          <button class="question-button${activeClass}" type="button" data-index="${index}">
            ...
            <small>${escapeHtml(question.prompt || "ScoreDecoder question")}</small>
  ...
  function renderCurrentQuestion() {
    ...
    elements.questionTitle.textContent = question.term ? capitalise(question.term) : "Untitled ScoreDecoder question";
    ...
    elements.areaSuggestion.textContent = ELEMENT_SUGGESTED_AREA[question.element]
      ? `Suggested: ${ELEMENT_SUGGESTED_AREA[question.element]}`
      : "";

    setCheckedRadio("decision", review.decision);
    setCheckedRadio("difficulty", review.difficulty);
    setCheckedValues("area", review.areas || []);
  ...
  function renderSourceSummary(question) {
    const rows = [
      ...
      ["Term", question.term],
      ["Symbol display", question.symbol_display],
      ...
      ["Requires score context", question.requires_score_context],
      ["Score context template", question.score_context_template],
      ["Audio required", question.audio_required],
      ["Audio file", question.audio_file],
  ...
  // --- Iframe audition: drives the REAL ScoreDecoder app via the same
  // teacher-load-question postMessage contract Live Sessions use (see
  // student/student.js's renderLiveIframeQuestion/sendLiveLoadQuestion and
  // shared/js/progress-embed-contract.js). Reusing this exact wire format
  // means what appears here is exactly what a student would see — no
  // separate re-implementation of ScoreDecoder's rendering to keep in sync. ---

  function buildSlotId(question) {
    return `review:${question.question_id}:${Date.now()}`;
  }

  // Reuses Progress Mode's own iframe styling (modules/progress-mode/app-drivers.js,
  // loaded alongside this script) so the audition panel shows the quiz card
  // full-size instead of the standalone page's setup/home screen sitting
  // above it in normal document flow — exactly what a Live Session iframe
  // looks like, not a fresh re-implementation of the same rules.
  ...
  function loadQuestionIntoIframe(question) {
    ...
    const sourceKey = ELEMENT_SOURCE_KEY[question.element] || "musical-language-tempo";
    elements.auditionFrame.dataset.slotId = slotId;
    elements.auditionFrame.dataset.sourceKey = sourceKey;
    elements.auditionFrame.dataset.questionId = question.question_id;
    elements.iframeStatus.textContent = "Loading ScoreDecoder…";
    ...
    elements.auditionFrame.onload = () => {
      ...
      applyFocusMode();
      if (appReadyForToken !== token) sendLoadQuestion(question, slotId, sourceKey, token);
    };
  ...
  function sendLoadQuestion(question, slotId, sourceKey, token) {
    if (token !== loadToken) return;
  ...
  function handleFrameMessage(event) {
    ...
    if (message.type === "app-ready") {
      ...
      applyFocusMode();
      const question = currentQuestion();
      if (question) sendLoadQuestion(question, slotId, elements.auditionFrame.dataset.sourceKey, loadToken);
      return;
    }
    ...
    if (message.type === "pool-empty") {
      elements.iframeStatus.textContent = "ScoreDecoder reported this question could not be loaded.";
      elements.iframeStatus.classList.remove("is-hidden");
      return;
    }
  }
  ...
  function downloadJson() {
    ...
    const payload = {
      ...
      source: "ScoreDecoder review.html",
      questionCount: questions.length,
      reviewedCount: rows.filter((row) => row.decision || row.reviewedLevel || row.areas || row.issues || row.notes).length,
      rows
    };
    downloadFile(`score-decoder-review-${dateStamp()}.json`, JSON.stringify(payload, null, 2), "application/json");
  }
  ...
  function downloadCsv() {
    ...
    downloadFile(`score-decoder-review-${dateStamp()}.csv`, csv, "text/csv");
  }
  ...
  function bindEvents() {
    ...
    elements.clearReviewButton.addEventListener("click", () => {
      if (!window.confirm("Clear all locally saved ScoreDecoder review notes?")) return;
      state.reviews = {};
  ...
  async function init() {
    try {
      ...
      if (!response.ok) throw new Error(`ScoreDecoder data failed to load (${response.status}).`);
      const text = await response.text();
      questions = parseCsv(text);
    } catch (error) {
      document.body.innerHTML = `<main class="panel"><h1>ScoreDecoder questions could not be loaded.</h1><p>${escapeHtml(error?.message || "")}</p></main>`;
      return;
    }

    if (!questions.length) {
      document.body.innerHTML = "<main class=\"panel\"><h1>ScoreDecoder questions could not be loaded.</h1></main>";
      return;
    }
